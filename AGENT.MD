...
- All automated tests live under `test/`, mirroring the runtime folders:
  ...
  - `test/routes/api` hosts HTTP-level tests; `orders.test.ts` seeds the test DB via the repository and asserts pagination + error logging flows as well as the placeholder `/api/orders/signatures` response, while `health.test.ts` exercises the new health endpoints.
