...
test("GET /api/keys exposes public keys with fingerprints", async (t: TestContext) => {
  ...
  t.assert.strictEqual(payload.hubId, app.hubKeys.hubId);
  t.assert.strictEqual(payload.current.kid, app.hubKeys.current.kid);
  t.assert.strictEqual(payload.current.publicKeyPem, app.hubKeys.current.publicKeyPem);

  const expected = createHash("sha256")
    .update(app.hubKeys.current.publicKeyPem)
    .digest("hex");
