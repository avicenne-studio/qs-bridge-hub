...
export function extractSignatureSlot(value: unknown) {
  if (!isObject(value)) {
...
export function extractErrorMetadata(event: unknown) {
  if (!isObject(event)) {
...
export default fp(
  async function wsSolanaListener(fastify: FastifyInstance) {
        ...
        void queue.push(async () => {
          try {
            if (decoded.type === "outbound") {
              ...
              await handleOutboundEvent(decoded.event, { signature, slot });
    ...
    const onOpen = () => {
      ...
      reconnectAttempt = 0;
      fastify.log.info(
        { wsUrl, usingFallback: isUsingFallback() },
    ...
    const scheduleReconnect = () => {
      ...
      if (isUsingFallback() && fallbackRetryTimer === null) {
        fastify.log.info("Switching back to primary WebSocket");
        wsUrl = config.SOLANA_WS_URL;
        reconnectAttempt = 0;
      } else if (!isUsingFallback()) {
        temporarySwitchToFallback();
        reconnectAttempt = 0;
      }
      ...
      reconnectTimer = setTimeout(() => {
        ...
        fastify.log.warn({ wsUrl, delayMs, usingFallback: isUsingFallback() }, "Reconnecting Solana WS");
        ws = wsFactory(wsUrl);
  ...
  },
  {
    ...
    dependencies: ["env", "events-repository"],
  }
);
